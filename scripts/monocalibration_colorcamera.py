    ...
    for nd in nd_list:
        ...
        for gray in gray_range:
            aeparams = mlcm.pyAEParams(dynamic_range=gray, target_max=gray +
                                       0.05, target_min=gray - 0.05, max_time=190000, rate=1000000)
            ...
            average_gray_X = process_image(
                X, image_x, image_y, roi_width, roi_height) + gray_offset
            process_channel(
            ...
            update_status(
                f"{mlcm.MLFilterEnum_to_str(nd_enum)}_X_channel_averageGray:{str(average_gray_X)}_exposureTime:{str(exposure_time)}")

            average_gray_Y = process_image(
                Y, image_x, image_y, roi_width, roi_height) + gray_offset
            process_channel(
                exposure_time=exposure_time,
            ...
            update_status(
                f"{mlcm.MLFilterEnum_to_str(nd_enum)}_Y_channel_averageGray:{str(average_gray_Y)}_exposureTime:{str(exposure_time)}")

            average_gray_Z = process_image(
                Z, image_x, image_y, roi_width, roi_height) + gray_offset
            process_channel(
                exposure_time=exposure_time,
            ...
            update_status(
                f"{mlcm.MLFilterEnum_to_str(nd_enum)}_Z_channel_averageGray:{str(average_gray_Z)}_exposureTime:{str(exposure_time)}")
    time.sleep(1)
    update_status("写入配置中...")
