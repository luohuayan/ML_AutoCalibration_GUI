...
def get_CIEu_CIEv(srXpath, srYpath, srZpath, blursize):
    ...
    u = 4 * CIEx/sumtemp
    v = 9 * CIEy/sumtemp
    return u, v, srcX, srcY, srcZ

if __name__=='__main__':
    rootpath = r'E:\CPYZ\M_06\color'
    savepath = rootpath + '/'
    blursize = 5
    JNCDshowMax = 1.5 # JNCD 绘图坐标的上限
    LvshowMin = 0.6     # Lv 绘图坐标的上限
    ...
    for loop_i in range(1, 5):
        ...
        ui, vi, srcXi, srcYi, srcZi = get_CIEu_CIEv(srXpathi, srYpathi, srZpathi, blursize)
        row, col = ui.shape[0], ui.shape[1]
        half_size = 1150
        center_row, center_col = int(row / 2), int(col / 2)
        JNCD_i = np.sqrt((ui - ui[center_row, center_col]) ** 2 + (vi - vi[center_row, center_col]) ** 2) / 0.004
        Lv_normal = srcYi/np.nanmax(srcYi)
        Lv_normal[Lv_normal==0] =np.nan
        ## 绘图01：Lv uniformity
        fig = plt.figure(figsize=(16, 10))
        plt.subplot(2, 2, 1)
        plt.imshow(Lv_normal, cmap='jet', vmin=LvshowMin, vmax=1)
        plt.xlabel('Col(pixel)')
        plt.ylabel('Row(pixel)')
        plt.colorbar(label="(Normalized Lv)")
        plt.title('Lv uniformity')
        plt.subplot(2, 2, 2)
        Along_col = Lv_normal[center_row, (center_col - half_size): (center_col + half_size)]
        Along_row = Lv_normal[(center_row - half_size): (center_row + half_size), center_col]
        plt.plot(Along_col)
        plt.plot(Along_row)
        plt.grid(which='both')
        plt.xlabel('Position(pixel)')
        plt.ylabel('Normalized Lv')
        plt.ylim([LvshowMin, 1])
        plt.title('Lv uniformity')
        plt.legend(['Along-Col', 'Along-Row'])
        ## 绘图02：JNCD uniformity
        plt.subplot(2, 2, 3)
        plt.imshow(JNCD_i, cmap='jet', vmin=0, vmax=JNCDshowMax)
        plt.xlabel('Col(pixel)')
        plt.ylabel('Row(pixel)')
        ...
        plt.xlabel('Position(pixel)')
        plt.ylabel('JNCD')
        plt.ylim([0, JNCDshowMax])
        plt.title('JNCD uniformity')
        plt.legend(['Along-Col', 'Along-Row'])

        pngname = 'Lv_JNCD_uniformity_loop' + str(loop_i) + '_blur_' + str(blursize) + '.png'
