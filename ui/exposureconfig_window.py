...
class ExposureConfigWindow(QDialog):
    ...
    def load_config(self):
        ...
        file_name, _ = QFileDialog.getOpenFileName(
            self, "Load Exposure Config", "", "JSON Files (*.json);;All Files (*)", options=options)
        if file_name:
            try:
                    ...
                    for nd in self.nd_list:
                        ...
                        for xyz in self.xyz_list:
                            ...
                            if nd_str in exposure_map and xyz_str in exposure_map[nd_str]:
                                ...
                                mode_combobox, time_entry = self.settings[(
                                    nd_str, xyz_str)]
                                mode_combobox.setCurrentText(
                                    setting['exposure_mode'])
                                time_entry.setText(
                                    str(setting['exposure_time']))
                            else:
                                mode_combobox, time_entry = self.settings[(
                                    nd_str, xyz_str)]
                ...
                QMessageBox.critical(self, "MLColorimeter", "Load config error: " + str(
                    e), QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)

    def save_config(self):
        options = QFileDialog.Options()
        file_name, _ = QFileDialog.getSaveFileName(
            self, "Save Exposure Config", "", "JSON Files (*.json);;All Files (*)", options=options)
        if file_name:
            try:
                for nd in self.nd_list:
                    nd_enum = mlcm.MLFilterEnum(int(nd))
                ...
                reply = QMessageBox.information(
                    self, "成功", "保存成功！", QMessageBox.Ok)

                if reply == QMessageBox.Ok:
                    self.close()
            except Exception as e:
                QMessageBox.critical(self, "MLColorimeter", "Save config error: " + str(
                    e), QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
    ...
    def closeEvent(self, event):
        if not self.is_config_saved:
            ...
            reply = QMessageBox.question(
                self, "MLColorimeter", "配置未保存，确定要关闭窗口吗？", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
