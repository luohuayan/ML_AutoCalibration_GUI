...
class FFCCalculateBinningWindow(QDialog):
    ...
    def _init_ui(self):
        ...
        self.line_edit_half_size.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        grid_layout.addWidget(self.line_edit_half_size, 7, 0)

        self.label_gray_range = QLabel(
            "灰度范围：(输入两个值表示灰度最小值和最大值，例如5 11，表示灰度范围为50%到110%)")
    ...
    def start_ffc_calculate(self):
        try:
            ...
            if not self.file_path:
                QMessageBox.warning(self, "MLColorimeter", "请选择FFC图像保存路径",
                                    QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
                return
            ...
            if self.selected_binning == "ONE_BY_ONE":
                self.ffc_calculate_1_thread = FFCCalculateBinningThread_1(
                    parameters)
                self.ffc_calculate_1_thread.finished.connect(
                    self.on_ffccalculate_finish)
                self.ffc_calculate_1_thread.error.connect(
                    self.on_ffccalculate_error)
                self.ffc_calculate_1_thread.status_update.connect(
                    self.update_status)
                self.ffc_calculate_1_thread.start()
            elif self.selected_binning == "TWO_BY_TWO":
                self.ffc_calculate_2_thread = FFCCalculateBinningThread_2(
                    parameters)
                self.ffc_calculate_2_thread.finished.connect(
                    self.on_ffccalculate_finish)
                self.ffc_calculate_2_thread.error.connect(
                    self.on_ffccalculate_error)
                self.ffc_calculate_2_thread.status_update.connect(
                    self.update_status)
                self.ffc_calculate_2_thread.start()
            elif self.selected_binning == "FOUR_BY_FOUR":
                self.ffc_calculate_4_thread = FFCCalculateBinningThread_4(
                    parameters)
                self.ffc_calculate_4_thread.finished.connect(
                    self.on_ffccalculate_finish)
                self.ffc_calculate_4_thread.error.connect(
                    self.on_ffccalculate_error)
                self.ffc_calculate_4_thread.status_update.connect(
                    self.update_status)
