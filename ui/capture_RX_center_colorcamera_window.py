    ...
    def start_capture(self):
        try:
            if self.exposure_map_obj == {}:
                QMessageBox.warning(
                    self, "警告", "请点击曝光时间按钮设置曝光时间", QMessageBox.Ok)
                return
            nd_enum = [int(nd)
            ...
            if not self.save_path:
                QMessageBox.critical(self, "MLColorimeter", "保存路径未设置",
                                     QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
                return
            self.status_label.setText(
                "<span style='color: green;'>状态: 正在拍图...</span>")  # 更新状态
            ...
            self.captureRXcenter_thread = CaptureRXCenterColorCameraThread(
                parameters)
            self.captureRXcenter_thread.finished.connect(
                self.on_captureRxcenter_finished)
            self.captureRXcenter_thread.error.connect(
                self.on_captureRxcenter_error)
            self.captureRXcenter_thread.status_update.connect(
                self.update_status)
