...
class CaptureFFCCalUniformityPlotColorCameraWindow(QDialog):
    ...
    def _init_ui(self):
        ...
        self.label_ndlist.setText(
            "nd列表, (4: ND0, 5: ND1, 6: ND2, 7:ND3, 8:ND4), 以空格隔开，输入一个以上的值，如4 5")
        ...
        self.label_xyzlist.setText(
            "xyz列表, (1: X, 2: Y, 3: Z, 10: Clear, 12: YA), 以空格隔开，输入一个以上的值，如1 2")
        ...
        self.label_capture_times = QLabel(
            "capture_times:(multi frame averaging)")
        ...
        self.line_edit_capture_times.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
    ...
    def _start_capture_calculate(self):
        try:
            ...
            self.vrange = [
                float(vrange) for vrange in self.line_edit_vrange.text().strip().split()]
    ...
    def start_calculate_uniformity(self):
        if self.cb_calculate_uniformity.isChecked():
            ...
            if not self.rx_dict:
                QMessageBox.warning(self, "MLColorimeter",
                                    "RX未配置", QMessageBox.Ok)
                self.btn_capture.setEnabled(True)
                self.is_running = False
                return
            if not self.uniformity_path:
                QMessageBox.warning(self, "MLColorimeter",
                                    "保存路径不能为空", QMessageBox.Ok)
                return
            exposure_mode = self.get_current_exposure_mode()
            exposure_time = float(self.line_edit_exposure_time.text())
            self.exposure = mlcm.pyExposureSetting(
