...
class FiledCurveThread(QThread):
    ...
    def run(self):
        try:
            field_curve(status_callback=self.status_update.emit,
                        **self.parameters)
...
class FiledCurveWindow(QDialog):
    ...
    def _init_ui(self):
        ...
        self.line_edit_binnlist.setPlaceholderText(
        ...
        self.line_edit_exposure_mode.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        ...
        self.line_edit_exposure_time.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        ...
        self.line_edit_focus_max.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        ...
        self.line_edit_focus_min.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        ...
        self.line_edit_focal_space.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        ...
        self.line_edit_use_fine_adjust.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        ...
        self.line_edit_freq_list.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        ...
        self.line_edit_roi_count.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        ...
        self.roi_display.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
    ...
    def add_roi(self):
        try:
            ...
            x, y, width, height = map(
                lambda w: int(
                    getattr(self, f"line_edit_{w}_input").text().strip()),
                ['x', 'y', 'width', 'height']
            )
    ...
    def load_config(self):
        try:
            ...
            file_name, _ = QFileDialog.getOpenFileName(
                self, "Load ROI Config", "", "JSON Files (*.json);;All Files (*)", options=options)
            if file_name:
                # 读取json文件
    ...
    def save_config(self):
        try:
            ...
            file_name, _ = QFileDialog.getSaveFileName(
                self, "Save ROI Config", "", "JSON Files (*.json);;All Files (*)", options=options)
            if file_name:
                # 序列化
                serialized_dict = []
    ...
    def start_capture(self):
        try:
            ...
            self.binn = mlcm.Binning(
                int(self.line_edit_binnlist.text().strip()))
            self.exposure_mode = self.get_current_exposure_mode()
            self.exposure_time = float(
                self.line_edit_exposure_time.text().strip())
            ...
            self.focal_length = float(
                self.line_edit_focal_length.text().strip())
            ...
            self.average_count = int(
                self.line_edit_average_count.text().strip())
            self.freq_list = [
                float(freq) for freq in self.line_edit_freq_list.text().strip().split()]
            if not self.freq_list:
                QMessageBox.critical(self, "MLColorimeter", "请填写频率列表",
                                     QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
                return
            if not self.roi_list:
                QMessageBox.critical(
                    self, "MLColorimeter", "请添加roi", QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
                return
            self.out_path = self.line_edit_path.text().strip()
            if not self.out_path:
                QMessageBox.critical(self, "MLColorimeter", "请选择保存路径",
                                     QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
                return
            ...
            self.status_label.setText(
                "<span style='color: green;'>状态: 正在进行拍图...</span>")  # 更新状态
            ...
            self.filed_curve_thread.finished.connect(
                self.on_filed_curve_finished)
