...
class MonoCalibrationColorCameraWindow(QDialog):
    ...
    def _init_ui(self):
        ...
        self.line_exposure_time.setValidator(self.double_validator)
        self.line_exposure_time.setText("100")
        self.line_exposure_time.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        ...
        self.label_luminance_lum.setText(
            "Luminance(请输入三个值，分别表示三个颜色通道X、Y、Z对应的亮度）：")
        ...
        self.line_edit_luminance_lum.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
    ...
    def start_mono_calibration(self):
        try:
            ...
            self.luminance_list = [float(
                luminance) for luminance in self.line_edit_luminance_lum.text().strip().split()]
            if len(self.luminance_list) < 3:
                QMessageBox.warning(self, "MLColorimeter",
                                    "请输入三个亮度值", QMessageBox.Ok)
                return
