...
class CaptureImageFixedLUMWindow(QDialog):
    ...
    def _init_ui(self):
        ...
        self.line_edit_etlist.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        ...
        self.line_edit_input_path.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        ...
        self.checkbox_color_shift_flag.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        ...
        self.checkbox_distortion_flag.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        ...
        self.checkbox_exposure_flag.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        ...
        self.checkbox_four_color_flag.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
    ...
    def capture_images(self):
        try:
            ...
            self.et_list = [
                float(et) for et in self.line_edit_etlist.text().strip().split()]
            ...
            if not self.nd_list or not self.xyz_list or not self.et_list or not self.save_path:
                QMessageBox.critical(self, "MLColorimeter", "请完整填写所有参数",
            ...
            if self.cb_useRX.isChecked():
                ...
                self.axis_list = [
                    int(axis) for axis in self.line_edit_axislist.text().strip().split()]
            else:
            ...
            self.captureimagefixedLUMafterFFCThread = CaptureImageFixedLUMafterFFCThread(
                parameters)
            self.captureimagefixedLUMafterFFCThread.finished.connect(
                self.on_capture_finished)
            self.captureimagefixedLUMafterFFCThread.error.connect(
                self.on_capture_error)
            self.captureimagefixedLUMafterFFCThread.status_update.connect(
                self.update_status)
