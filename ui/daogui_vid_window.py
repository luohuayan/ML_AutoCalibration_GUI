...
class DaoGuiVIDWindow(QDialog):
    ...
    def _init_ui(self):
        ...
        self.line_edit_vid_list.setText(
            "166 200 250 333 400 500 600 700 800 900 1000 1300 1600 2000")
        self.line_edit_vid_list.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        ...
        self.line_edit_inf_pos.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        from_layout.addRow(self.label_inf_pos, self.line_edit_inf_pos)

        self.label_pos_offset = QLabel(
            "pos_offset:(公式infinity_position + 1000 / vid * 0.8 中的这个1000，减法请输入负数)")
        ...
        self.line_edit_pos_offset.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        from_layout.addRow(self.line_edit_pos_offset)

        self.label_coef = QLabel(
            "coef:(公式infinity_position + 1000 / vid * 0.8 中的这个0.8)")
        ...
        self.line_edit_coef.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        ...
        self.line_edit_focal_length.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        from_layout.addRow(self.label_focal_length,
                           self.line_edit_focal_length)
        ...
        self.line_edit_pixel_size.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        ...
        self.line_edit_use_chess_mode.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        ...
        self.line_edit_use_lpmm_unit.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        ...
        self.line_edit_rough_step.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        ...
        self.line_edit_freq.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        ...
        self.line_edit_average_count.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        from_layout.addRow(self.label_average_count,
                           self.line_edit_average_count)
        ...
        self.line_edit_total_pulse.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        ...
        self.btn_browse.clicked.connect(lambda: self._open_folder_dialog(
            self.line_edit_path))  # 添加lambda保证按钮在点击时再调用
        ...
        self.line_edit_roi_set_path.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        from_layout3.addRow(self.label_roi_set_path,
                            self.line_edit_roi_set_path)
        self.btn_roi_set_browse = QPushButton("浏览...")
        self.btn_roi_set_browse.clicked.connect(
            lambda: self._open_folder_dialog(self.line_edit_roi_set_path))
        ...
        self.line_edit_roi_path.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        from_layout3.addRow(self.label_roi_path, self.line_edit_roi_path)

        self.btn_roi_browse = QPushButton("浏览...")
        self.btn_roi_browse.clicked.connect(
            lambda: self._open_folder_dialog(self.line_edit_roi_path))
        ...
        self.line_edit_mtf_path.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)
        from_layout4.addRow(self.label_mtf_path, self.line_edit_mtf_path)
        self.btn_mtf_browse = QPushButton("浏览...")
        self.btn_mtf_browse.clicked.connect(
            lambda: self._open_folder_dialog(self.line_edit_mtf_path))
    ...
    def start_test_daogui(self):
        try:
            self.status_label.setText(
                "<span style='color: green;'>状态: 正在运行</span>")  # 更新状态
            ...
            QMessageBox.critical(
                self, "MLColorimeter", e, QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
    ...
    def on_thread_finish(self):
        ...
        self.status_label.setText(
            "<span style='color: green;'>状态: 移动完成！</span>")  # 更新状态
        self.btn_test.setEnabled(True)
    ...
    def start_capture_image(self):
        try:
            ...
            self.vid_list = [
                int(vid) for vid in self.line_edit_vid_list.text().strip().split()]
    ...
    def on_capture_finished(self):
        ...
        self.status_label.setText(
            "<span style='color: green;'>状态: 拍图完成！</span>")  # 更新状态
        self.btn_vid_image.setEnabled(True)
        self.is_running = False  # 标识完成
    ...
    def start_roi_config(self):
        try:
            ...
            start_generate_roi_config(
                roi_set_path=self.roi_set_path, roi_out_path=self.roi_out_path)
            QMessageBox.information(
                self, "MLColorimeter", "生成结束!", QMessageBox.Ok)
    ...
    def start_vid_test(self):
        try:
            if self.line_edit_roi_path.text() is None:
                QMessageBox.warning(
                    self, "MLColorimeter", "请先选择roi配置保存路径(即roividconfig.json所在路径)", QMessageBox.Ok)
                return
            self.roi_out_path = self.line_edit_roi_path.text()
